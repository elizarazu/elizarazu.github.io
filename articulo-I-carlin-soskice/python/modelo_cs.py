...
El sistema se escribe como z_{t+1} = A z_t, con z_t = (x_t, pi_t)'.
"""
import numpy as np


def matriz_transicion(alpha, gamma, kappa, eta, s):
    """Construye la matriz de transición del modelo."""
    ...
    return np.array(
        [[a, -gamma * phi_pi],
         [kappa * a, 1.0 - kappa * gamma * phi_pi]],
        dtype=float,
    )


def indicadores_dinamicos(alpha, gamma, kappa, eta, s):
    """Calcula traza, determinante, discriminante y valores propios."""
    ...
    return {
        "A": A, "T": T, "D": D, "Delta": Delta,
        "lambda_1": eig[0], "lambda_2": eig[1],
        "rho": np.max(np.abs(eig)),
    }


def respuesta_demanda(alpha, gamma, kappa, eta, s, periods=40):
    """Genera la respuesta ante un shock unitario de demanda."""
    if periods < 1:
        raise ValueError("periods debe ser un entero positivo.")

    A = matriz_transicion(alpha, gamma, kappa, eta, s)
    z = np.zeros((periods + 1, 2), dtype=float)
